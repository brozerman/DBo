...
class DBoStaticTest extends PHPUnit_Framework_TestCase {
	...
	public function testObject() {
		...
		foreach (DBo::object("SELECT c FROM test.t1 WHERE a=?", [1]) as $o) {
			$this->assertInstanceOf("DBo", $o);
	...
	public function testBuildData() {
		$arr = ["a"=>10 "b"=>20, "c"=>"hello", "invalid"=>"world"];
		$obj = DBo::t1()->setFrom($arr);
		unset($arr["invalid"]);
	...
	public function testGet() {
		...
		$q = [null, "SELECT a.* FROM test.t1 a WHERE a.a=1 LIMIT 1", "SELECT a.c FROM test.t1 a WHERE a.a=1 LIMIT 1"];
		for ($i=0; $i<2; $i++) {
			$this->assertEquals(DBo::t1(1)->c, "ab");
			$this->assertEquals(end(mysqli_log::$queries), next($q));
		}
		// utilize used columns in second run
		$q = [null, "SELECT a.* FROM test.t1 a", "SELECT a.c FROM test.t1 a"];
		for ($i=0; $i<2; $i++) {
			foreach (DBo::t1() as $o) $o->c;
			$this->assertEquals(end(mysqli_log::$queries), next($q));
		}
	}
