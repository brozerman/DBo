...
class DBoStaticTest extends PHPUnit_Framework_TestCase {
	...
	public static function setUpBeforeClass() {
		...
		$db->query("DROP TABLE IF EXISTS t1,t2,t3,t4");
		$db->query("CREATE TABLE t1 (a INT, b INT, c VARCHAR(20))");
		$db->query("INSERT INTO t1 VALUES (1,2,'ab'),(3,4,'cd'),(5,6,'ef');");
		$db->query("CREATE TABLE t2 (a INT AUTO_INCREMENT PRIMARY KEY, b VARCHAR(20))");
	...
	public function testInsertUpdateDeleteReplace() {
		DBo::query("CREATE TABLE test.t5 (a INT AUTO_INCREMENT PRIMARY KEY)");

		$id = DBo::query("INSERT INTO test.t5 VALUES ()");
		$this->assertEquals($id, "1");

		$id = DBo::query("INSERT INTO test.t5 VALUES ()");
		$this->assertEquals($id, "2");
		$this->assertEquals(DBo::query("UPDATE test.t5 SET a=a-1"), 2);

		DBo::query("INSERT INTO test.t5 VALUES ()");
		$this->assertEquals(DBo::query("DELETE FROM test.t5"), 3);

		DBo::query("INSERT INTO test.t5 VALUES ()");
		$this->assertEquals(DBo::query("REPLACE INTO test.t5 VALUES (4)"), 4);
		DBo::query("DROP TABLE test.t5");
	}
	...
	public function testLoadSchema() {
		DBo::hello();
		$schema = (object)[
			"col"=>["test"=>["t1"=>["a"=>1, "b"=>1, "c"=>1],"t2"=>["a"=>1,"b"=>1],"t3"=>["a"=>1,"t2_a"=>1],"t4"=>["a"=>1,"b_arr"=>1,"c_json"]]],
	...
	public function testBuildQuery() {
		...
		$this->assertEquals(DBo::t2(0)->buildQuery(), "SELECT a.* FROM test.t2 a WHERE a.a='0'");
		...
		$this->assertEquals(DBo::t2(0)->t3()->buildQuery(), "SELECT a.* FROM test.t3 a WHERE a.t2_a='0'");
		...
		$this->assertEquals(DBo::t2([[1,2],[3,4]])->t3()->buildQuery(), "SELECT a.* FROM test.t3 a WHERE a.t2_a IN (1,3)");
		...
		$this->assertEquals(DBo::t2(42)->buildQuery("UPDATE", null, "set"), "UPDATE test.t2 a SET set WHERE a.a=42");
	}
	...
	public function testOvalues() {
		...
		$this->assertEquals(DBo::t2([59,60])->ovalues("a"), ["59","60"]);
	}
